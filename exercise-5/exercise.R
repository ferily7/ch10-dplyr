...
average.delay <- group_by(flights, month) %>% 
...
# Hint: you'll have to perform a grouping operation before summarizing your data
airport.highest.delay <- group_by(flights, month) %>% summarise(delay = mean(dep_delay, na.rm = TRUE)) %>% 
                          filter(delay == max(delay)) %>% select(month)
